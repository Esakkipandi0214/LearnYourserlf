...
export default function TestsPage() {
  ...
  useEffect(() => {
    ...
    const fetchTests = async () => {
      try {
        const response = await fetch("/api/tests/getTests/6795fce2f5a55a767b18523a");
  ...
  return (
    <Layout>
      <div className="space-y-6 p-6">
        ...
        <h1 className="text-xl flex justify-center items-center font-bold text-[#23486A] ">Qeustion Bank</h1>
        <button onClick={()=>router.push("/Create-test")} className=" bg-red-600 flex text-sm justify-center items-center font-medium px-1 py-2 rounded-lg text-white">Create Test</button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {tests.map((test) => (
            ...
            >
              <h2 className="text-xl font-semibold text-white">{test.TestTitle}</h2>
              <p className="text-white/70">
                Questions: <span className="font-semibold">{test.questions.length}</span>
              </p>
